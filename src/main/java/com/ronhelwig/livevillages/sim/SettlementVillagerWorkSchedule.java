...
public final class SettlementVillagerWorkSchedule {
	...
	static final long VILLAGE_GATHERING_START_TICK = 9_000L;
	...
	public static boolean shouldStartNewWork(ServerLevel level, Villager villager, String workKey, long decideIntervalTicks) {
		return isProfessionWorkTime(level)
			&& !SettlementChurchWork.isChurchSabbathRest(level)
			&& !isTakingBreak(level, villager)
			&& isDecideTurn(level, villager, workKey, decideIntervalTicks);
	}
	...
	public static boolean isProfessionWorkTime(ServerLevel level) {
		...
		return dayTime < VILLAGE_GATHERING_START_TICK && !SettlementChurchWork.isChurchSabbathRest(level);
	}
