...
- At the end of the village workday, Miners who are still underground should return back up the shaft toward the surface instead of remaining parked at depth overnight.
- Planned tunnel rules:
  ...
  - `Primary Tunnel`: `3` blocks tall, `1` block wide, with `Copper Lanterns` overhead about every `10` blocks when available or wall torches as fallback
