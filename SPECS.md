...
- `Butcher`: the vanilla `Smoker` profession slot, expanded for herd growth, culling, leather production, sheep shearing, pasture expansion, and settlement-wide livestock husbandry rather than only near the `Smoker`; player-placed `Smoker` blocks should anchor a staged `Butcher Shop` when a valid nearby footprint is available, and first-pass trade outputs should include beef and mutton surpluses when the settlement has enough managed herd capacity.
