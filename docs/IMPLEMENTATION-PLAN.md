...
- miner evening return should use bounded multi-step ladder/tunnel movement, exact ladder-position preference, side-tunnel routing back to an existing ladder, rescue ladders for bedrock-level shaft traps, and throttled surface-return diagnostics so deep miners can surface within the gathering/rest window instead of stalling one block above a ladder segment or stranded in a branch
...
- extend loaded Miner work beyond the starter shaft by deepening only until the top bedrock layer, then cutting upper-level-first `2`-wide by `3`-tall primary side tunnels to the settlement radius about every `5` levels with `2` solid levels between them; once a level's primary tunnels are full length, add perpendicular `1`-wide by `2`-tall secondary tunnels to the settlement radius with wall torches every `9` blocks, temporary `dirt` / `cobblestone` floor supports when needed, exposed-face ore scanning so visible resources are not left untouched, discovery signs for notable features, and monster-closure signs that block only the unsafe primary branch
