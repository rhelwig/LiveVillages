...
- make the first-pass `Tier 1` palisade concrete: log wall at least `4` blocks high, top-half interior slab firing walk below the top log, torch spacing about every `10` blocks, stair access about every `30` blocks, and simple path-aligned gatehouses
...
- add first-pass `Palisade Gatehouse` support: iron-bar and copper-bar variants, eight-way placement/preview including diagonals, radius-percent preview feedback, staged gatehouse blueprint, and later wall segments between completed gatehouses near the `80%` radius
