...
- `Trade Board`, `Scribe Desk`, and bakery sale / `Glass Display Case` colors now come from a shared tier-aware settlement screen theme that is chosen when the screen opens; open screens keep their current theme across refresh packets, while closing and reopening picks up any settlement tier change
- virtual `Trading Post` materialization now ignores unrelated or wrong-settlement `Trade Board` blocks when deciding whether an eligible mature loaded settlement still needs its first linked board/post
...
- blocked anchored workstation structures now update or place one nearby `can't build` sign with a concise blocker reason from the central structure validation path, rather than repeatedly placing generic signs during loaded retry passes
...
- add loaded-world materialization for virtual improvements, especially Trading Posts completed by settlements before the player first loads the area; first pass can place a linked Trade Board and start the normal staged Trading Post build for mature loaded settlements that lack both
