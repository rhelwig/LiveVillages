...
- When a player donates a usable bakery ingredient through the bakery sale screen, that ingredient should be swept out of the visible sale display promptly enough that the player does not keep seeing it linger in the sale stock as if it were still for sale.
...
- At `Banner Bearer` or better, nearby outpost raiders should aid the player against recent hostile monster attackers while the player is inside that outpost's influence. This is defensive solidarity, not full command authority.
...
- A loaded outpost should only raid when it has enough raiders to send a party while leaving a guard behind, should not raid more than about once per Minecraft week, and should choose a nearby non-outpost settlement rather than a random far-away target.
- The first-pass raid flow should be observable and persistent: `Muster` announces the target to `Banner Bearer+` players, `March` represents the party traveling toward the target, `Raid` attempts control or tribute resolution, `Return` brings surviving participants back to their outpost, and `Cooldown` prevents repeat launches. Visible participants should be actual outpost members rather than unrelated vanilla raid waves.
...
- Loaded raiders selected for a visible outpost raid party should be tagged as active members of their home outpost while they are away, remain counted in that outpost's observed population during the march/raid, and use dry land waypoints that prefer existing paths or road surfaces instead of blindly taking the shortest line through nearby water.
...
- A village may pay tribute to stop a raid before major damage. Tribute should be smaller than a successful sack, roughly `10-15%` of transferable stock value, while a successful sack may transfer up to roughly `20-25%`, capped by raiding party capacity and biased toward outpost-valued goods such as emeralds, ingots, arrows, food, leather, wood, weapons, armor, and tools.
...
- A successful actual raid may also take villagers as `Thralls`; do not use the term `slavery` in feature text. Thralls become part of the outpost population and are removed from their home village, traveling back with surviving raiders when visible return behavior exists. The number taken should never exceed the lesser of half the raider count and `20%` of the victim village population. Unemployed villagers should be taken first, then professionals. Thralls working a profession in the outpost should be as effective as ordinary unemployed village helpers; unemployed Thralls should be less effective than ordinary unemployed villagers.
