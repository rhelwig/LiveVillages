...
- Once the main shaft reaches bedrock, Miners should cut primary side tunnels from both ladder sidewalls, starting from the upper eligible levels and proceeding downward. Primary tunnels should be `2` blocks wide and `3` blocks tall, extend to the owning settlement's base radius, and keep `2` solid rock levels between tunnel levels so the vertical cadence remains about every `5` blocks. They should scan all exposed tunnel air cells for useful ore such as redstone instead of only checking the walking block. When a tunnel mouth would otherwise lack footing, they may place a temporary or permanent `dirt` / `cobblestone` floor block from settlement stock to create a standable work spot.
...
- If a Miner exposes an interesting underground feature such as a geode, sculk site, or ancient-city indicator, the Miner should mark the primary tunnel with a wall sign naming the discovery. Discovery signs do not close the tunnel, and a tunnel can have both a discovery sign and a monster-closure sign.
