...
- A settlement with a staffed `Trade Board` may be renamed from the board's `Civic` tab. Renaming should require the same settlement-management permission used for demolition approval, validate length and allowed characters, reject names already used in that dimension, and show a preview before confirmation.
- A successful rename must update persistent settlement state and every dependent display or lookup, including placed Trade Board labels, menus, reports, route summaries, Milepost labels, commands, and future maps, without changing the settlement's stable internal id.
...
- Settlements should not stop planning when immediate survival needs are met. Planning priority should flow from shortages and safety-critical needs into growth capacity, profession infrastructure, routes, civic upgrades, and finally lower-priority prestige or defensive wants. Until the next tier population target is reached, civilian settlements should keep a modest housing reserve ahead of current population so growth can continue instead of waiting until the last bed is occupied.
- A developed Tier 1 settlement should autonomously want a `Roadwright's Workshop` / `Surveyor's Table`, even when current stock is healthy, so it can connect internal destinations and pursue reachable settlement routes. This civic ambition follows essential food, housing, and material support but is not dependent on a current construction shortage.
- Settlement planning should sample local elevation at a bounded cadence. Large height ranges and repeated steep changes raise both autonomous `Roadwright's Workshop` priority and Roadwright recruitment priority, so mountain settlements obtain terrain and access specialists earlier than flat settlements.
- Difficulty changes defensive priority rather than deleting defensive ambition. Peaceful settlements pursue Guards, Fletchers, and fortifications later and at a lower density; Easy, Normal, and Hard progressively raise their urgency. Creative player mode must not disable settlement growth or defense planning, because a Creative player may be preparing a world for later Survival play.
...
- Tier 1 uniforms should communicate scarce founding conditions with muted cloth, grime, abrasion, visible patches and repair stitches, and occasional frayed or uneven hems. Later tiers should progressively clean and tailor the same recognizable role identity rather than replacing it.
- The Fletcher's role silhouette includes a forward-pointed green medieval forester's cap with a visible red feather. Its finish should progress with civic tier, from patched and worn at Tier 1 through restrained silver and gold trim at later tiers, without obscuring the villager's eyes.
...
- Outpost illagers should also show civic-tier kit upgrades. A `Pillager` keeps its martial identity while trim and badges grow with the owning outpost's persisted tier. `Vindicator` and `Evoker` follow the same direction in concept art and HD base skins.
- Villager, zombie villager, and illager texture scale is a per-world game rule, `live-villages:villager_texture_scale`. It is constrained to `64`, `128`, or `256` through a cycling game-rule button and matching `/gamerule` command choices. `64` is the vanilla-size atlas, while new worlds default to `256` for the strongest presentation. Higher scales keep the same UV layout and add job marks so professions stay readable at a distance.

### Settlement Lighting

Settlement lighting should be treated as useful infrastructure first and decoration second. The settlement should prefer lighting doors, paths, workstations, docks, gates, bridges, farm edges, guard posts, and the `Trade Board` / `Trading Post` before adding purely decorative prestige lighting.

- `Tier 1`: survival lighting. Settlements should use known `Torch` and `Campfire` recipes for house doors, rough worksite edges, mine entrances, gathering spaces, and critical civic blocks. `Lantern` is allowed at Tier `1` only when the settlement already knows the recipe through observation, player contribution, or route trade; generated villages that already contain at least one placed vanilla `Lantern` near village POIs should record `minecraft:lantern` as settlement-known Scribe knowledge.
- `Tier 2`: established village lighting. Settlements should prefer upgrading interior torches to `Candle` or `Lantern` light where placement support exists. `Jack o'Lantern` should be used for farms, scarecrow-style farm markers, pumpkin fields, and rustic agricultural lighting. `Soul Torch` and `Soul Lantern` are accent lights for outposts, graveyards, shrines, serious guard spaces, and other ominous or memorial areas when Nether materials are known or imported.
- `Tier 3`: specialized and imported lighting. Settlements may use `Glow Berries` for garden trellises, living lampposts, market awnings, and civic greenery once discovered or traded. Harbors and waterfront civic spaces may use `Sea Lantern` as a Tier `3+` prestige/harbor light. `Glowstone` can act as imported civic light where a settlement has Nether access or trade.
- `Tier 4`: prestige and engineered lighting. Settlements should pursue superior or luxury light sources such as `Copper Bulb`, `Redstone Lamp`, `End Rod`, `Shroomlight`, and rare `Froglight` accents only after learning or trading the relevant recipe/resource knowledge. These should be used for wealthy storefronts, civic halls, central streets, towers, route beacons, trading districts, and high-value public spaces rather than ordinary starter houses.

Lighting recipes and lighting-pattern knowledge should use the Scribe/route knowledge model where possible. Real Minecraft recipes such as `candle`, `jack_o_lantern`, `lantern`, `soul_torch`, `soul_lantern`, `glowstone`, `sea_lantern`, `copper_bulb`, `redstone_lamp`, and `end_rod` can live in the Scribe recipe ledger. Non-crafted or discovery-based lighting such as `Glow Berries`, `Shroomlight`, and `Froglight` should use future settlement discovery knowledge rather than fake recipe IDs.

Structure blueprints should distinguish concrete blocks from semantic intent slots. Existing concrete symbols such as wall torches and hanging lanterns may remain valid, but tier-aware structures need slots such as `interior_light`, `storefront_light`, `farm_light`, `graveyard_accent`, and `harbor_light`. A slot should resolve to concrete planned blocks by tier, settlement kind, biome/material style, known recipes, route knowledge, stock, and placement support, and the resolved choice should be saved with the build site so in-progress construction does not change unpredictably when stock changes later. The same slot model should remain general enough for future semantic slots beyond lighting.

Lighting slots may resolve to different block arrangements. A wall torch is a one-block wall-mounted solution. A candle may need a support block, pot, decorated pot, ceramic base, shelf, table, or similar planned/existing support. A hanging lantern requires ceiling support. Jack o'lanterns, sea lanterns, shroomlights, froglights, copper bulbs, and redstone lamps are full-block placements. Tier `4` redstone lighting should normally be authored with supporting circuitry in the tier blueprint; redstone lamps should not be chosen for ordinary slots unless the slot or surrounding blueprint also plans power. Later single-unit powered lights may be modeled as combined fixtures, similar to a candle plus base, so a structure can use engineered lighting without exposing a full circuit in every small building.

Settlements should know flower-to-dye and dye-mixing recipes as ordinary overworld knowledge. Flowers and all sixteen dyes are trade goods. Villages keep a modest flower reserve and a small dye reserve, with extra `red_dye` for church work. Gardeners raise those reserves further. A village can make `red_dye` from flowers or beetroot when construction needs it.

Graveyards should become a later civic structure family. Whenever a settlement villager dies, for any reason, the settlement should create or extend a graveyard and place a grave marker there. Named members should receive a named stone such as `Here lies\nBob`. Unnamed members still get a marker. Gravestone style may vary by role: important civic figures such as `Trademaster` can receive larger two-block markers, while professions such as `Roadwright` can receive role-themed carved designs. If the settlement has a church, the graveyard should sit close to it as a churchyard rather than an isolated field across town. If graves are needed before a church exists, found the cemetery on the inland / opposite-dock side of the village so a later chapel can stand beside it. Graveyards should not be only decoration; they should provide a civic-memory, comfort, morale, or local-history benefit while also creating a natural place for `Soul Torch` and `Soul Lantern` accent lighting.
...
- Settlement-aware custom screens should use a tier-aware theme: `Tier 1` can stay rough and muted, while later tiers should become more colorful, refined, and polished. Theme colors should be centralized so they are easy to edit or later expose through configuration/data files. The theme should be snapshotted when the player opens a screen; if the settlement tier changes while the screen is open, that open screen should keep its current theme until closed and reopened. Pure recipe-utility screens may keep a vanilla-like layout when they do not present settlement identity or settlement status, but screens that already use Live Villages custom chrome or settlement map/state data should resolve the linked settlement tier when available.
...
### Civic Administration and Demolition

The Trade Board should have a `Civic` tab for settlement administration. Its first responsibilities are renaming the settlement and creating, reviewing, cancelling, and tracking demolition orders. Trade and bounty controls should not remain visible while this tab is active.

- A `Blank Demolition Order` is a craftable paper item. The first recipe should be shapeless: one `Paper` plus one `Ink Sac` produces one blank order.
- A blank order has no authority by itself. From the `Civic` tab, an authorized player selects one registered settlement structure, reviews its type, location, current use, occupants, workstation, storage, replacement consequences, and estimated recoverable materials, then asks the Trademaster to stamp the order.
- Approval requires a staffed, reachable Trademaster unless a server operator or Creative-mode management action explicitly bypasses that requirement. The approved item becomes a `Demolition Order` bound to the settlement id and exact persistent structure/build-site id, with the structure name, approval time, and approver recorded for display and diagnostics. It cannot authorize demolition of a different structure.
- Settlement-management permission must be server-configurable. Trademaster approval is a gameplay requirement, not a substitute for multiplayer access control; players who cannot manage that settlement must not be able to rename it, approve an order, cancel its projects, or post its demolition notices.
- An approved order is placed against a block belonging to its bound structure as a small portrait-oriented wall notice, visually related to a tiny item frame or posted paper but clearly distinct from ordinary decoration. Placement fails with an explanation if the targeted block is not part of the bound structure, the structure is already being demolished, or the notice cannot attach safely.
- Posting the notice creates a persistent demolition project and immediately suppresses ordinary repair and tier-retrofit work for that structure. Merely holding, dropping, or crafting an order must never suppress repairs. If a player begins dismantling the posted structure manually, workers should recognize those missing blocks as authorized demolition rather than trying to replace them.
- A posted demolition project should expose clear states such as `posted`, `paused`, `dismantling`, and `complete` in the `Civic` tab, project lists, overlays, and diagnostics. The structure should stop contributing beds, comfort, security, trade, production, route, sanctuary, storage-capacity, or other completed-building effects at the earliest safe point for each effect; dangerous or exploitable benefits must not remain active merely because one decorative block is still standing.
- Settlement workers may dismantle an authorized structure as a staged visible job. They should work from safely reachable upper and attached blocks downward, remove gravity-sensitive or hanging details before their supports, preserve stable walking/escape positions, avoid dropping themselves from roofs, and use the same bounded per-pass scheduling and assignment rules as construction.
- Worker demolition should target only blocks owned by the registered structure plan that still match the authored block or a compatible construction substitution. It must not clear the old blueprint's air/margin cells, surrounding roads, terrain, gardens, later player additions, or neighboring structures merely because they overlap the original bounding box.
- Containers, workstations, beds, doors, double blocks, block entities, fluids, redstone, and shared walls require explicit handling. Container contents must be deposited into settlement stock or safely dropped before the container is removed. Workers must not duplicate double-block drops or dismantle a block that is also registered as required infrastructure for another surviving structure without surfacing a conflict for player review.
- Worker-recovered blocks and useful drops should become carried goods and be deposited visibly into the Trade Board, Trading Post, or warehouse ledger. Salvage should derive from blocks actually removed and their normal recoverable output rather than crediting the blueprint estimate in advance. Player-demolished blocks remain ordinary player drops unless the player donates them.
- Removing the posted notice pauses worker demolition but does not erase the persistent order or silently re-enable repairs. The `Civic` tab should offer explicit `Resume`, `Cancel demolition`, and, where meaningful, `Restore structure` choices. Cancelling before material dismantling begins may simply reactivate the completed site; cancelling after work begins should either queue a normal repair/rebuild from the remaining world state or leave the site intentionally paused until the player chooses restoration.
- When no owned required blocks remain, the demolition project completes, its repair/build-site record and dependent assignments are retired, displaced workers and residents are released for reassignment or rehousing, and the cleared plot becomes eligible for greenspace or a new structure. Demolition does not automatically flatten terrain or erase roads; separate site-clearing or landscaping work may follow.
- First-pass demolition should support completed Live Villages staged structures. Generated vanilla buildings, arbitrary player buildings, overlapping composite structures, palisade networks, bridges, docks with shared route function, and imported blueprints should remain ineligible until they have reliable ownership and dependency records.
- A Trademaster may later propose and post an autonomous demolition order when a settlement-authored structure is obsolete, duplicated, blocks a clearly higher-priority replacement, or is deliberately being retired for greenspace. Autonomous demolition must be conservative: never target arbitrary player or vanilla structures, occupied essential housing, the last critical workstation, active sanctuary/defense infrastructure during danger, or storage containing goods; show the proposal publicly for at least one in-game day, provide a clear player veto at the Trade Board, and require a validated replacement/relocation plan when an essential capacity would be lost.
- Demolition planning must participate in settlement queues and duplicate suppression. A site cannot be repaired, upgraded, rebuilt, and demolished simultaneously, and a replacement structure must not claim the plot until demolition has reached the safe completion state.
...
- Future `Fletching Table` work should consider utility and martial arrow variants beyond material arrowheads:
  - `Torch Arrow`: crafted from a normal arrow plus a fuel-tip ingredient such as `coal` or `charcoal`; when it lands on a valid block face it should consume the projectile and place a torch or wall torch, failing harmlessly or dropping an item if normal placement rules cannot be satisfied.
  - `Explosive Arrow`: crafted from a normal arrow plus a `Fire Charge`; on impact it should use damage and blast behavior no stronger than the closest vanilla Fire Charge projectile behavior and must stay clearly weaker than a TNT block.
- If a separate installed mod exposes an oil or lamp-fuel item, Live Villages may accept that item as an alternate `Torch Arrow` tip through tags or compatibility data, but base Live Villages should keep the non-mod dependency path available through coal and charcoal.
- Villages that can produce, stock, trade, donate, or use these arrow variants should treat the finished arrows and their tip ingredients as settlement goods so Fletcher trades, Guard ammunition reserves, Trade Board wants, and pricing do not lose track of them.
...
- A valid staged structure footprint may bridge a modest slope by planning dirt foundation columns from stable natural ground to immediately beneath the structure floor. Those support blocks are part of ordinary progressive construction, must be built before unsupported upper layers, and may span no more than five blocks of empty vertical space; deeper gaps remain invalid sites.
...
- Build-site completion and progress displays should count current required structure blocks, not obsolete clearance / air cells. Foundation dirt fill, optional tiles such as a reserved brewing stand or copper bell, bed occupancy, and auto-connected stair or fence shapes must not keep a finished building incomplete. When every required authored block is present, the site should persist `complete=true`. A finished site should not reopen just because a later blueprint edit added cells; repair should reopen only when a required block is actually missing from the world. An incomplete build site should not display `100%`; `100%` should be reserved for sites that are complete or structurally complete by required-block status.
...
- Goods still needed by unfinished staged construction should be reserved. The Trade Board may show extra stairs, slabs, or similar parts as surplus only after the active build sites' remaining demand is covered, and it should not sell the reserved amount.
- Construction workers should recover misplaced beds that occupy a non-bed planned cell, then continue the planned block. They should not stay stuck on an uncleared blocked cell when other supported work remains on the same site.
- Emergency beds address actual homelessness, not the settlement's growth-reserve target. The decision must use currently registered local HOME POIs rather than a delayed persisted housing count, and placement must have a settlement-wide cooldown that worker productivity cannot compress. Villagers should first try a planned permanent bed slot in an active build site; if that slot's floor is missing and the settlement has the materials, they should finish that floor and then place the bed there. While a Housing Shelter or Simple Housing Shelter remains incomplete, outdoor fallback is suppressed so construction retains priority. If no housing build is active, a genuinely homeless settlement may place a spaced outdoor fallback with clear standing/access cells outside unfinished construction footprints. Existing outdoor beds must not be automatically removed because the mod cannot reliably distinguish its old emergency beds from player-placed beds.
...
- Blueprint metadata should eventually support optional per-cell material preferences, similar in spirit to orientation layers. A designer could mark a default stone-family wall cell as preferring `stone`, `granite`, `diorite`, `andesite`, `mossy_cobblestone`, or another compatible stone variant so the final build can use textured wall mixes when settlement stock allows it.
- Material preference layers should preserve the construction intent family rather than changing the structure category: stone-family preferences stay stone-like, wood-family preferences stay wood-like, and logs/planks/stairs/fences should keep their functional form while allowing compatible species or texture choices.
- If a preferred material is unavailable, construction may fall back to the blueprint's default material or biome palette material rather than blocking the build indefinitely. These preferences should make structures more interesting, not turn decoration into a hard requirement.
...
- `Trading Post` repairs and tier upgrades must preserve the `Trade Board` while replacing the block under it, and loaded construction maintenance should restore the linked board at the build site's planned `W` cell if an earlier support replacement already dropped it. If an earlier repair restored the board at a stale saved anchor/workstation position instead, maintenance should move it back to the planned `W` cell rather than leaving a duplicate or off-footprint board.
...
The Trade Board displays the linked settlement name directly on the placed board, similar in purpose to sign text but not implemented as a vanilla sign.
...
- The board text color should vary by settlement tier so the placed block reflects civic progression without requiring the player to open the screen.
...
- A zero-population civilian settlement should not be permanently dead if it still has enough supplies. Stored food, basic safety, and available housing should slowly build recovery progress toward attracting an outside recruit; a custom settlement may attract one recovery caretaker from food stock even before it has completed housing, but additional growth should require real housing capacity. Recovery should use the persistent overworld day clock so sleeping, time advancement, and server restarts do not leave abandoned settlements stuck on session-local retry timers.
...
- A loaded Carpenter may also process excess log stock at the `Carpenter's Bench`, maintaining useful reserves of planks, slabs, stairs, sticks, fences, fence gates, doors, trapdoors, chests, and ladders without consuming logs below the village's log reserve target. Finished parts beyond the basic reserve set should be produced when settlement or active-construction demand exists rather than stockpiled without a consumer.
...
Bridges are route infrastructure. Roadwrights and Masons should be able to maintain and upgrade them as ordinary corridor work, and a later player-placed designation block may also direct workers to start a specific crossing.

- If a craftable bridge-direction block is added, placing it should start or resume a staged bridge from that marker in the block's facing direction, the same way a `Lighthouse` or shelter marker starts a staged build. The marker's recipe ingredients should credit the site. Invalid placements should show the existing `can't build here` / red preview feedback rather than failing silently.
- Maximum buildable span is tied to the settlement's civic tier. A higher-tier village may attempt a longer crossing; a lower-tier village must refuse or wait when the gap is longer than it can currently span.
- Bridge construction, width, and height should also improve with tier. Early bridges are short, narrow, and simple. Later bridges are longer, wider, taller, and more finished, using the same material ladder as roads where practical.
- Suggested first-pass limits, to tune after playtest:

| Civic tier | Max span | Deck | Construction |
| --- | --- | --- | --- |
| 1 | about `8` blocks | `2` wide, deck at path height, little or no railing | wood / dirt-path language, simple supports |
| 2 | about `16` blocks | `3` wide, low rails | cobble piers, sturdier deck |
| 3 | about `24` blocks | `3–4` wide, taller rails or sides | taller structure, smooth-stone language |
| 4 | about `32` blocks | `5` wide, taller and more imposing | dressed stone / brick language, heavier piers or arches |

- Span is the unsupported or over-gap length, not including land abutments.
- An existing shorter or narrower bridge should be eligible for in-place widening, raising, and material upgrades when the settlement's tier advances, rather than requiring a second parallel crossing.
- Autonomous Roadwright corridor work may still bridge small gaps on its own. The designation block is for player-chosen crossings and for spans that should not start unless someone pointed at them.
...
- Autonomous dock construction should not repeatedly build extra docks just because completed dock planks cover the original water-surface columns. Completed autonomous docks should leave a completed dock build-site record, and infrastructure scans should still recognize existing dock piers whose first shoreline rows sit over solid shore while the pier end reaches deep water.
...
A lighthouse is a harbor structure rather than a workstation. It should be visually simple in the early tier and readable from a distance. Later civic-tier upgrades should make the same tower both larger and rounder so it reads as a real lighthouse rather than a thicker square chimney. Design sizes and silhouettes are in [LIGHTHOUSE-DESIGN.md](LIGHTHOUSE-DESIGN.md).
...
- Field professions may have a managed work range beyond the base settlement radius. First-pass bounds are up to `150%` of settlement radius for `Miner`, `Fisherman`, `Forester`, and `Portmaster`, and up to `200%` of settlement radius for `Roadwright`, with a bounded additional relay interval when a placed `Milepost` clearly continues an established route. Census and night behavior should still treat assigned villagers in that range as belonging to their settlement.
- Outside-radius travel must remain profession- and goal-specific rather than becoming a general permission for villagers to roam. `Foresters` may seek harvestable trees, `Fishermen` may reach usable shore or dock water, `Miners` may reach their Mine Entrance and connected underground workings, `Portmasters` may reach docks, lighthouses, and harbor approaches, and `Roadwrights` may work along surveyed roads, bridges, and Milepost-marked inter-settlement routes. Other ordinary production professions should remain inside the settlement and express missing resources through settlement or trade demand.
- `Shepherds` and `Butchers` may make temporary bounded excursions beyond the settlement radius only when a completed fenced pen lacks suitable breeding stock: Shepherds may locate and lead sheep home, while Butchers may locate and lead cows or pigs home. The excursion should retain a remembered animal and destination pen, time out safely, avoid repeatedly claiming the same animal, and end with an evening return.
- `Beekeepers` may make a temporary bounded bootstrap excursion when a completed local apiary has no viable bee population. They may locate bees or a natural bee nest, lure bees home with flowers, and establish them in a local managed hive. They must not destroy or relocate an occupied natural nest unless the behavior preserves its bees safely; once the apiary is viable, routine Beekeeper work should remain local.
- `Gardeners` may make temporary bounded bootstrap or planting excursions when a completed chicken pen lacks a breeding pair or village garden stock lacks useful local flowers. They may lead chickens home with seeds and gather or transplant a limited number of naturally occurring flowers into village gardens without stripping an area bare. They should prefer renewable propagation from existing settlement flower stock once a useful local variety is established.
- `Guards` may cross the settlement boundary only for a concrete defensive duty such as escorting an assigned worker or caravan, patrolling an established route corridor, pursuing an immediate threat, or responding to a raid. This is not a general gathering or wandering radius. Future Trademaster or caravan travel should likewise be tied to an explicit caravan rather than ordinary daily roaming.
- Every outside-radius acquisition task should use bounded loaded-world searches, cached or remembered targets, explicit return destinations, timeouts and target release, and normal evening return behavior. Unemployed helpers should remain settlement-local unless a separate role explicitly assigns them to an outward task.
- If the settlement cannot fit the associated structure around a player-initiated workstation or structure anchor, villagers should place a nearby temporary sign that says `can't build here` rather than silently ignoring the station. Ambient scans of vanilla village workstations should not place those signs inside generated structures.
- While the player is holding a recognized workstation item whose placement would anchor an associated structure or dock, the existing `O` build-preview toggle should also show the prospective footprint before placement: green when valid, red when blocked. For workstation-style items such as `Carpenter's Bench`, `Stonecutter`, `Smoker`, or `Fletching Table`, this prospective structure preview should only appear when the placement is outside an existing sheltered structure; placing one inside an existing building is treated as adding a lone workstation to that building. This should work inside known outposts as well as normal settlements.
...
- Completed staged structures should receive bounded material retrofits as settlement tier increases instead of requiring duplicate replacement buildings. First-pass structure material tiers are `Cobblestone` with ordinary logs at `Tier 1`, `Stone` with stripped logs at `Tier 2`, and `Stone Bricks` with stripped logs at `Tier 3+`; workers should reopen the existing build-site blocks and consume matching settlement stock one block at a time. When replacing an existing structure block, the old block should be recovered into settlement stock before the replacement material is consumed, so upgrades can use the recovered material as recipe input when applicable.
- Tier-specific structure blueprints should take priority over simple material retrofits when they exist for the current structure and settlement tier. For example, a `Tier 2` `Trading Post` with an authored `Tier 2` blueprint should upgrade toward that blueprint, while a structure without a matching tier blueprint, such as a `Forester's Hut`, should fall back to the simple material retrofit for that tier.
...
- Completed workstation-associated structures should not be treated as permanently frozen. If a required block is removed, loaded construction maintenance should reopen that same build site promptly and stage the repair instead of requiring a fresh workstation placement. Blueprint-only growth (new cells added to the Java plan after the building already finished) must not silently reopen a completed site; later civic-tier upgrades remain an explicit upgrade path.
...
- `Forester`: harvests and replants wood resources, must not cut down trees that contain bee nests or bee hives, maintains a sparse tree presence inside the village, manages denser woodland beyond the village core, and keeps a diverse seedling reserve that can create sapling trade demand. Foresters prioritize natural trees occupying reserved active construction sites and do not replant those cleared footprint trees. General planting and harvest discovery must use a shared, incremental outward search rather than a separate full-area scan for every Forester. Foresters are surface workers: they should not choose underground dropped-item tasks, descend mine shafts, or remain on loaded mine shaft columns when the mine-rescue logic can move them back to the surface. Their profession overlay should stay woodland-readable at every civic tier, but the uniform should upgrade with the settlement so a higher-tier Forester looks more established and more aligned with that tier's theme.
...
- `Dock`
- `Lighthouse`
- `Altar` as the church founding workstation
- `Church` / `Cathedral`, sited opposite the dock when the village chooses the plot
- `Graveyard` / churchyard, kept next to the church, with a marker for every villager death
- a later bridge-direction designation block, with span and size capped by civic tier
- `Scribe Desk`
...
- `Glass Display Case` block and item models should follow the `Baker's Counter` display language: material-specific frames and rear panels, glass viewing panes, two visible shelves, and dedicated item/inventory transforms so carried and inventory views show the authored display-case shape clearly.
...
- As the shaft reaches eligible depths, Miners should cut primary side tunnels from both ladder sidewalls, starting from the upper eligible levels and proceeding downward instead of waiting for the whole shaft to reach bedrock. The highest primary tunnel should sit immediately below the authored `Mine Entrance` starter shaft: the tunnel is `3` blocks tall, with its top aligned one block below the entrance blueprint's lowest shaft level, and later primary tunnels repeat every `5` blocks downward from that alignment. Primary tunnels should be `2` blocks wide and extend to the owning settlement's base radius, with `2` solid rock levels between tunnel levels. Mine-shaft wall torches should be placed on the wall opposite the ladder at the same vertical level as each primary tunnel floor. They should scan all exposed tunnel air cells for useful ore such as redstone instead of only checking the walking block. When a tunnel mouth or tunnel floor would otherwise lack footing, they may place a temporary or permanent `dirt` / `cobblestone` floor block from settlement stock to create a standable work spot, preferring lower-value blocks such as `dirt` before spending cobblestone.
...
- When a primary or secondary horizontal tunnel breaks into a cave, ravine, open underground space, or above-ground opening, Miners should secure the breach instead of leaving the settlement mine network open. The Miner should place a door at the tunnel-side boundary of the opening where it controls access to the tunnel, then fill surrounding gaps around the door frame and nearby tunnel edge so there are no one-block, diagonal, or baby-zombie-sized paths around it. If glass blocks are available in abundant settlement stock, glass is the preferred filler because workers and players can see hostile mobs beyond the door; otherwise low-value solid filler such as `dirt` or `cobblestone` is fine, with `dirt` preferred before spending cobblestone when both are acceptable.
...
- Passive active-site preview messages should require the player to be near the actual unfinished work area instead of only near the owning settlement footprint. First-pass active-site previews use a shorter `64` block 3D radius from the nearest unfinished preview block, anchor, workstation, or origin, while explicit held-item and targeted placement previews may remain longer-range.
...
- `Sling`: a fast, short-range bow-class ranged weapon with built-in unlimited low-power stone ammunition. The first pass registers a lightweight craftable item with a Balearic-style two-cord leather-pouch model, lets players fire `2`-damage sling stones on a `12`-tick cooldown at about `75%` of full-bow launch velocity, uses it for visible Gardener and Shepherd presentation, and lets loaded Gardeners and Shepherds fire weak line-of-sight sling shots during bell-response defense at about `80%` of Guard bow range with a faster cadence than bows.
...
- `Scythe`: a slow iron tool-weapon that hits harder than a `Crooked Staff` but is weaker overall than proper swords due to low attack speed. It should deal `4` attack damage, use `1.2` attack speed, carry iron durability/repair/enchanting behavior, use custom long-haft curved-blade item art, let players right-click mature crops to harvest and replant them while reserving one seed/root item from the drops, let players trim grass-like plants and leaf litter, and give loaded Farmers a defined close-defense weapon.
- Settlements recognize `Sling`, `Crooked Staff`, `Scythe`, and `string` as Tier 1 goods. Villages reserve slings for Gardeners and Shepherds, crooked staffs for Shepherds, and scythes for Farmers so Trade Board wants and donations align with profession equipment.
- One wool can be spun into four string, reversing vanilla's four-string-to-wool craft. Players unlock that recipe by picking up wool. Settlement construction and stock conversion can spin wool when string is needed, so Peaceful worlds and shepherd-started villages are not locked behind spiders.
...
- `Anvil`, `Chipped Anvil`, and `Damaged Anvil` blocks are Smithy support infrastructure, not job-site anchors. An anvil placed inside or immediately beside a completed `Smithy` should improve the shared Smithy by letting `Armorers`, `Toolsmiths`, and `Weaponsmiths` complete two normal recipe batches per production action when the settlement has enough inputs; it should not create a separate profession or start its own structure.
...
- Housing pressure should create demand for finished `Bed` stock as well as wood and stone materials, so a settlement with `Shepherd`, `Forester`, and `Carpenter` support can produce wool, planks, beds, and housing without player-supplied bed blocks.
...
- The `Duplex` is the next shared-housing upgrade: a two-story version of the `Housing Shelter` with an exterior staircase running along one side, a second-floor front porch above the unobstructed lower entrance, and two beds on each floor, for four beds total.
- When a settlement crosses upward into a higher civic tier, its registered housing should advance exactly one step: `Simple Housing Shelter` sites become `Housing Shelter` sites and `Housing Shelter` sites become `Duplex` sites. A partially built site keeps its compatible placed work when the larger plan is reconciled. This marking is persisted and must not repeat merely because the settlement remains at that tier. If the settlement later drops a tier and re-attains it, housing added during the lower-tier period becomes eligible on that new upward transition.
- Autonomous housing-pressure projects must start the same ordinary staged build sites as player-placed shelter markers. Both the larger shelter and the emergency/simple fallback must be constructed block by block rather than appearing instantly.
- Players should be able to craft dedicated `Simple Housing Shelter` and `Housing Shelter` placement blocks. When one is placed successfully, that block position becomes the shelter's lower door block and the placed facing becomes the shelter entry orientation. The placement should start or resume an ordinary staged build site rather than placing the whole shelter instantly, so villagers can construct it over time, pause on missing materials, and finish later. The crafted block's recipe components should be treated as contributed build materials for that shelter placement, with any remaining shortfall coming from settlement stock as available. Loaded settlement maintenance should also retry already-placed shelter markers that do not yet have a matching build site, preserving the marker facing and applying the same one-time recipe-material contribution when the retry creates the site. Once door materials are available, villagers should clear the placed shelter marker promptly and replace it with the planned lower door block rather than leaving the marker in the completed doorway.
...
- Completed `Cleric` churches should add civic-pride comfort that scales with the church's civic tier, from a small chapel bonus to a cathedral-sized one. Design sizes are in [CLERIC-CHURCH-GUIDE.md](CLERIC-CHURCH-GUIDE.md). Only the highest completed church on that site should count. A staffed church (at least one `Cleric`) adds a further small comfort bonus.
- If a settlement has a completed church and a `Cleric`, villagers hold a service every eight days on the full moon, about an hour after breakfast, for one hour. The first cleric stands behind the `Altar`, a second stands behind the `Pulpit`, and everyone else gathers in the nave. After the service they take the rest of the working day off and enjoy trees and gardens until evening gathering. A church `Copper Bell` rings a few times to call the meeting and does not raise a defense alarm.
- The interior of a completed church is sanctuary. If the victim is inside the nave, vestry, or bell-tower shaft, intentional harm does no damage. Environmental damage and block breaking are unchanged. Mobs drop targets that enter or stand in the church.
- When the village itself places a church, it should prefer the opposite side of the settlement from the primary dock so the church and the harbor lighthouse sit at opposite ends of town and the skyline reads as balanced from afar. If there is no dock, ordinary civic placement is fine. Player-placed altars or brewing stands may ignore that preference. If the ideal opposite-dock plot is blocked, use the next-best valid site rather than stalling the church.
- Most professions should have a public building that also supplies their beds. `Farmer`, `Fisherman`, and `Portmaster` are exceptions and use ordinary village housing. A later Farmer barn and a Tier 3+ lighthouse bedroom are optional worksite / harbor lodging, not required houses.
- An unattended village should be able to start every profession structure it needs from overworld materials. Player-placed workstations are a shortcut, not a requirement.
- `Cleric` churches found on a craftable `Altar`. The altar recipe is one `Stone`, one gold nugget, one wool, and one red dye. The block is slightly shorter than a full cube, and a tapestry drapes over the top and down the front with a red heart. A `Pulpit` is later sanctuary furniture, not the founding lock: four wooden slabs, one plank, one wool, and one red dye, shaped like a lectern with a wider pedestal and the same heart tapestry hanging down the front. Both the `Altar` and the `Pulpit` are cleric job sites, one villager each. A `Brewing Stand` remains the potion workstation. In a well-outfitted chapel the altar sits in the center, the pulpit on one side, and a brewing stand on the other. Villages with clergy should strongly want that brewing stand for potion work once blaze rods, nether wart, or trade make one available.
- A `Copper Bell` is a copper-only craftable bell. Players can craft it from six copper ingots, a stick, and a plank. Unattended villages need a `Mason` to cast one into stock. Tier 3 and 4 church front towers reserve a bell tile; the church can finish without it.
- Vanilla 26.1 has cut copper stairs, but not full-block copper stairs. Live Villages adds `Copper Stairs` and `Waxed Copper Stairs` using the copper-block texture. Players craft them from copper blocks or wax them with honeycomb. Unattended villages can cut them at a Mason from a copper block. Tier 3 and 4 church tower roofs use waxed copper stairs so the roof does not oxidize and reopen the site.
...
- `Captain` rank should unlock fuller raid command authority: forcing or denying tribute deals, negotiating deals, and later target/mission control beyond the first-pass goat-horn raid request.
...
- When a visible outpost raid member dies during an active raid, nearby `Banner Bearer+` players should receive a chat message naming the illager type, home outpost clan, and raid target so player-allies can follow raid losses without relying only on entity counts or logs.
...
- Players with cheats enabled, and players in Creative mode, may use `/tp <settlement name>` or `/teleport <settlement name>` to visit a known settlement. Settlement names should autocomplete case-insensitively. Arrival should use an open position near the settlement's bell when present, then its Trade Board, then its recorded center, and must fail instead of placing the player in an unsafe cell.
...
- Loaded farmers should treat every nearby `Composter` as a managed farm-territory anchor, including player-placed composters that are not the farmer's assigned job-site POI, and roam that territory rather than idling near only a tiny starter bed.
- When a loaded settlement wants more farming capacity, autonomous composter placement should cover existing farm areas first instead of placing duplicate composters around one farm. Only after existing farms have composter coverage should the settlement start a new starter farm area, roughly `6x6` to `10x10`, with a preferred log boundary, farmland interior, water near the center, and a composter near that water. A populated village with no Farmers still desires one starter farm so the first composter can exist and recruit that first Farmer; extra sites then follow the usual one-site-per-three-Farmers rule, capped at six. The first starter farm may be planned even while housing is still one bed short, so villages do not stall in a no-farmer / no-farm loop. Extra farms wait until housing pressure is cleared. Starter farm creation should spend stocked logs on border blocks when available, but missing border logs should fall back to ordinary dirt rather than preventing the farm from being created.
- Autonomous starter farms should be buildable by villagers from ordinary settlement materials, should avoid roads and existing farms, and should use a single finished level. Workers may clear or fill modest natural terrain within the same cut/fill limits used by ordinary structure siting rather than requiring the whole footprint to be perfectly flat before work starts.
- Holding or targeting a lone `Composter` with the active build preview should show the prospective starter farm footprint, or an invalid red footprint with a short reason and blocker highlight if no buildable farm layout fits around that composter. Valid prospective farm previews should use the same held-item coloring as staged structure previews so logs, dirt/farmland, water buckets, and the composter cell are distinguishable.
- A player-placed or moved lone `Composter` in a valid open spot inside a loaded farming settlement should act as an intentional farm anchor. If it is not already near an existing farm area, Farmers may spend stocked logs to build the same starter farm around that existing composter instead of placing another composter, and pending autonomous composter projects should retire rather than place another composter when these lone anchors already satisfy farm-site demand.
...
- A lone `Trademaster` in a custom settlement with housing pressure should do slow bootstrap civic work: convert a few stored logs into planks if needed, weave donated wool and planks into a bed, and place that bed when a full housing shelter cannot start yet. They should not match a `Shepherd` or `Carpenter`, but they must not stand idle while the materials sit in stock.
- Autonomous support workstations (`Forester`, `Carpenter`, `Miner`) should still start when a settlement needs beds, even if starter or donated wood and stone already meet generic stock targets. Missing housing is demand.
- Once a civilian settlement has at least four residents, an unbuilt autonomous-support workshop that includes beds should normally satisfy housing pressure before the settlement starts or materializes another plain housing shelter. This preference gives the village both housing and useful profession capacity; already-progressed housing projects may wait rather than being discarded.
- A settlement whose bounded terrain survey detects powder snow should equip its residents and new village recruits with leather boots as basic civic cold-weather gear, so ordinary settlement growth does not repeatedly reset through avoidable freezing deaths.
- Mine Entrance reconciliation may accept an existing ladder with another facing and an existing log with another pillar axis as satisfying the intended cell. Workers and previews should not remove, replace, or mark those blocks missing solely to normalize those orientation properties.
- Foundation fill must be derived from the blueprint at each fill elevation, not merely from the rectangular footprint. An authored air/excavation, ladder, or structural cell must take precedence over generic dirt support. In particular, Mine Entrance shaft columns remain open, dirt intruding into an authored shaft-air cell is removed, and ladder-backing cells are repaired to their authored solid support rather than accepted as dirt.
...
- The first autonomous civic improvement for a non-outpost settlement should be its `Trading Post`: abstract simulation should queue and complete that intent before later civic construction, and loaded catch-up should place a linked `Trade Board` before starting the normal staged `Trading Post` build.
- Until abstract projects carry exact physical anchors for every civic structure, a loaded settlement with population but no linked `Trade Board` and no `Trading Post` build site may choose a valid nearby dry footprint, preferring candidates near the village bell when one is loaded and falling back to settlement-center candidates, place a linked `Trade Board`, and start the normal staged `Trading Post` build as a bounded catch-up materialization pass. Generated `Trade Board` / `Trading Post` catch-up sites should strongly prefer having the board, porch, and entrance face back toward the bell or settlement center, with the structure extending away from that civic anchor. Legacy settlements without first-week starter materials should be topped up enough to bootstrap that first `Trade Board` / `Trading Post` attempt rather than remaining blocked by pre-MVP empty or depleted stock.
- After catch-up changes are applied to the world, resurvey and reset derived settlement state from reality so virtual state does not drift away from the actual village. Catch-up materialization should be block/state reconciliation only: it may place linked workstations, create or update build-site records, and persist stock changes, but it should not recruit workers, assign villager homes, path villagers, run roadwork, or advance loaded construction workers in the same maintenance pass.
...
- Each settlement stores a `lastUpdateTime` based on the persistent overworld day clock, while loaded-world worker throttles may still use short-lived server session ticks.
...
- Worker productivity is a per-world game rule, `live-villages:worker_productivity`. It is a multiplier on loaded worker maintenance cadence, daily work-unit caps, and abstract project work rates. `1.0` is the original design baseline. The default is `2.0`, which is twice as fast and is the intended normal play speed. Allowed values range from `0.25` to `50.0`. Loaded maintenance cadence may shrink with higher multipliers but should not run more often than every `4` ticks. Players should be able to change it from the world game-rules screen or with `/gamerule`.
- Productivity also scales construction discovery, home assignment, and roadwork planning so fast workers do not finish a task and then wait on an unscaled planner. Broad placed-workstation reconciliation remains separately throttled to a minimum interval of 15 seconds because it is substantially more expensive than ambition planning.
- Optional compatibility layers can support:
  - alternate currencies
  - guard or villager-role mods
  - road or structure mods
  - ship or cargo mods
  - fuel, pipe, and lighting mods such as Flame Fuel Farm And Forge and Simple Copper Pipes
- With compatible fuel/pipe/lighting mods installed, Live Villages should eventually be able to recognize a village lighting network as useful infrastructure: farmers maintain crops and compost inputs, composters or related blocks produce oil/fuel, pipes route that fuel to storage and lanterns, and the settlement treats the resulting maintained lighting as a safety/comfort improvement rather than as a hard dependency.
- This compatibility should stay optional and data-driven. Without those external mods, Live Villages should continue to use vanilla-compatible torches, lanterns, and ordinary settlement lighting behavior.
...
- Loaded civilian settlements progressively renovate generated vanilla village buildings through bounded scans and visible worker movement, assigning up to six nearby workers concurrently. Cobweb cleanup takes first priority and only targets cobwebs inside recorded vanilla village structure pieces. Functional renovation runs ahead of general structural restoration while housing is below population plus one: it may restore a clearly missing half of an existing wooden door or restore a bed only where that building's intact vanilla template authored one. Both bed halves must be missing, supported, roofed, clear overhead, and reachable from an adjacent two-block-high standing cell. Restoration consumes a donated finished bed first and otherwise spends three wool and three planks. Housing beyond the original building capacity uses the normal staged housing system instead of placing beds heuristically in generated structures. A deliberately slower safety pass maps zombie-village pieces to their intact vanilla counterparts, respects the saved piece rotation, and fills only air where the intact template expects a material-recognized wall, roof, stair, slab, fence, door, or window-pane block. Each placement consumes the corresponding settlement good; unsupported upper blocks wait for lower repairs. The same pass restores intact-template farmland where ordinary dirt, grass, coarse dirt, or rooted dirt remains. Renovation never replaces an existing solid block, removes a player block, or restores template air. Full-fidelity decorative reconstruction remains outside this pass.
- A dedicated `Live Villages` creative inventory tab exposes every registered mod block item, tool, weapon, and arrow, with the `Trade Board` as its icon.
...
- Trade Board joining the settlement whose actual radius contains the board, or founding a custom settlement when the board is outside all settlement radii and not within the custom-settlement spacing limit of another non-outpost settlement. Founding previews still render outside settlements, and too-close founding attempts surface a visible placement message instead of silently linking across open terrain. Placed Trade Boards render the linked settlement name on both broad faces and tint that text by settlement tier.
- Placed-workstation anchoring for `Carpenter's Bench` -> `Carpenter's Workshop`, `Surveyor's Table` -> `Roadwright's Workshop`, `Trade Board` -> `Trading Post`, and player-placed vanilla `Cartography Table` -> `Cartographer's House`, including nearby `can't build here` signs for blocked player-initiated footprints when a sign spot is available. Held-item previews for workstation-style items suppress the prospective structure footprint when the target placement cell is already inside an existing sheltered structure, since that interaction means adding a lone workstation to the building rather than requesting a new associated structure. Ambient loaded maintenance scans of vanilla village workstations suppress those warning signs and clean up adjacent Live Villages `can't build` signs so generated village interiors are not marked as bad player placements. Valid player-placed workstations now create or resume persistent build-site records instead of instantly placing the associated structure, and loaded maintenance retries already-placed custom workstation anchors when no matching build site exists, including Carpenter's Workshops, Scribe Offices, Guard Posts, Gardener's Sheds, Beekeeper's Apiaries, Bakeries, Mine Entrances, Roadwright Workshops, and Forester Workshops. Loaded Tier 1 villages rank autonomous support workstations from current stock and active build-site material shortages: `Forester's Table` for logs and wood demand, `Carpenter's Bench` for plank/woodwork demand, and `Miner Workstation` for cobblestone, fuel, and iron demand. The support bootstrap places at most one new support workstation/site per discovery pass, suppresses player-facing blocked-placement signs, avoids rooftops and existing sheltered structures, spaces candidates away from existing support workstations and build-site footprints, caches clearance data for the candidate pool once per materialization pass, and caps blocked autonomous support build-site startup attempts before retrying from a later candidate cursor. Carpenter and Forester may each recruit one worker before the workshop bed is complete so construction can become self-supplying; priority workforce recruitment may also reclaim a settlement-owned vanilla professional whose current profession no longer has valid local demand, preventing new settlements from staying stuck with mismatched workers such as a Fisherman with no usable local fishing work. Build sites can dry-run material availability so planned blocks can be marked `pending` or `missing_material`. Loaded construction maintenance assigns nearby adult non-Nitwit villagers to claim one pending planned block at a time, walk toward it, clear natural obstructions, visit a known `Trade Board` or `Trading Post` anchor to retrieve or auto-craft needed supplies, and place the block; correctly player-placed planned blocks are recognized as `player_placed`. Compatible wrong-species or starter-stone form matches count as structure intent matches without being rewritten to the planned material, assisted placement keeps the player's compatible material while copying required placement properties, and worker placement may use compatible local material states for generic construction stock. Completed staged `Trading Post` sites provide local and route trade output bonuses, while incomplete `Trading Post` sites do not; incomplete `Carpenter's Workshop` build sites are tracked as in-progress rather than counted as completed workshop infrastructure. Trading Post repairs and upgrades preserve or restore the planned Trade Board `W` cell, cleaning up stale restored boards at old saved anchor/workstation positions when needed. Structure blueprints can now carry optional orientation rows, currently used by the `Cartographer's House` roof stairs so stair rotation does not depend only on edge heuristics; orientation-sensitive blocks such as stairs, slabs, doors, torches, fences, fence gates, beds, and logs require exact blockstate matches during construction reconciliation except where a compatible material substitution preserves the same construction intent.
...
2. Formalize settlement advancement after MVP: add persistent settlement-tier state, unlock tracking, recipe/knowledge state, starter knowledge, knowledge trading, tier-aware autonomous build rates, and tier-aware structure/path/fortification upgrades. For the first MVP publication, tier work should be limited to Tier 1 behavior plus visible settlement-screen theme changes.
...
4. Flesh out the Trade Board UX: expose knowledge goals, fuller project details, clearer price/offer information, route quality/safety/capacity, Trading Post upgrade state, profession-specific trade entry points, and a `Civic` tab for validated settlement renaming plus structure-bound demolition approval, cancellation, and progress.
...
9. Expand construction queues: add real defensive works, road/path placement and upgrades, dock/harbor projects, warehouse projects, Trading Post and Shopping Mall projects, workstation-linked building jobs, maintenance, settlement-wall projects, tier-aware build-rate caps and retrofit upgrades, and safe staged demolition with repair suppression, dependency checks, and actual-block salvage.
10. Finish land-route gameplay: nearby Roadwright-built corridors, Surveyor's Table and Milepost rules, internal path upgrades with comfort impact, path connectors to workstation buildings so they count in the local trade network, trade-weight bonuses from connected workstations and civic trade improvements, material-based road upgrades, tier-capped bridge rules including an optional player-facing bridge-direction block, named and direction-facing Milepost presentation with optional distance text, route maintenance, cartographer support for long-distance trade, and route security from guards and hostile pressure.
