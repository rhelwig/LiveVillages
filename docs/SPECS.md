...
- When a loaded settlement wants more farming capacity, autonomous composter placement should cover existing farm areas first instead of placing duplicate composters around one farm. Only after existing farms have composter coverage should the settlement start a new starter farm area, roughly `6x6` to `10x10`, with a preferred log boundary, farmland interior, water near the center, and a composter near that water. First-pass autonomous farm-site demand should be bounded by site capacity, roughly one farm site per three Farmers and capped at six farm sites, so mistaken or duplicate Farmer recruitment cannot snowball into a workstation field. Starter farm creation should spend stocked logs on border blocks when available, but missing border logs should fall back to ordinary dirt rather than preventing the farm from being created.
