...
Graveyards should become a later civic structure family. When a named settlement member dies, the settlement should be able to create or extend a graveyard and place a named gravestone, for example `Here lies\nBob`. Gravestone style may vary by role: important civic figures such as `Trademaster` can receive larger two-block markers, while professions such as `Roadwright` can receive role-themed carved designs. Graveyards should not be only decoration; they should provide a civic-memory, comfort, morale, or local-history benefit while also creating a natural place for `Soul Torch` and `Soul Lantern` accent lighting.
...
- Build-site completion and progress displays should count current required structure blocks, not obsolete clearance / air cells. An incomplete build site should not display `100%`; `100%` should be reserved for sites that are complete or structurally complete by required-block status.
