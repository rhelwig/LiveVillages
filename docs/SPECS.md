...
- When a loaded settlement wants more farming capacity, autonomous composter placement should cover existing farm areas first instead of placing duplicate composters around one farm. Only after existing farms have composter coverage should the settlement start a new flat farm area, roughly `6x6` to `10x10`, with a log boundary, farmland interior, water near the center, and a composter near that water. First-pass autonomous farm-site demand should be bounded by site capacity, roughly one farm site per three Farmers and capped at six farm sites, so mistaken or duplicate Farmer recruitment cannot snowball into a workstation field.
- Autonomous starter farms should be buildable by villagers from ordinary settlement materials, should avoid roads and existing farms, and should require a single flat level rather than terracing across uneven ground.
- A player-placed or moved lone `Composter` in a valid flat open spot inside a loaded farming settlement should act as an intentional farm anchor. If it is not already near an existing farm area, Farmers may spend stocked logs to build the same flat starter farm around that existing composter instead of placing another composter, and pending autonomous composter projects should retire rather than place another composter when these lone anchors already satisfy farm-site demand.
