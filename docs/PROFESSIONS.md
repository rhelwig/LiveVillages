...
- Later field behavior: when a completed fenced pen lacks breeding stock, makes a bounded temporary excursion beyond the settlement radius to locate and lead cows or pigs back to that remembered pen, then returns by evening
...
- Later field behavior: when a completed fenced pen lacks breeding stock, makes a bounded temporary excursion beyond the settlement radius to locate and lead sheep back to that remembered pen, then returns by evening
...
- Workstation capacity: supports `1` Beekeeper by default; a second Beekeeper should only be useful for an unusually large village with many managed hives. A placed `Honey Separator` bootstraps that first Beekeeper even before the apiary hut has beds.
- Notes: a village with a spare Trademaster may reassign that villager when a reachable `Honey Separator` has no Beekeeper
...
- Later field behavior: when local chicken pens lack a breeding pair or village gardens lack useful flower varieties, makes a bounded temporary excursion to lead chickens home with seeds or transplant a limited number of natural flowers; avoids stripping wild flower patches and prefers renewable propagation once the settlement has starter stock
...
- Recipe/testing note: the craftable `Guard Post` workstation recipe is `IFI / PPP / S S`, where `I` is `Iron Bars`, `F` is any fence, `P` is any plank, and `S` is a stick. The recipe should unlock for testers who pick up common guard-post materials such as iron ingots or iron bars, fences, planks, or sticks.
...
- Donations: first-pass code lets players donate swords, armor, shields, ranged weapons, ammunition, banners, and similar spoils of war directly to a `Guard Post` by sneak-using the post with one eligible item in the main hand; offhand items are not consumed by this direct donation path. Banner donations enter settlement stock as `Desecrated Enemy Banner` trophies for later display
