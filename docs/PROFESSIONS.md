...
- Loaded-world behavior: first-pass staged construction covers the Mine Entrance and starter shaft, and the first loaded Miner loop extends that completed shaft one level at a time while consuming ladders, descends through the shaft to the current reachable ladder rung in loaded villages before working, lights the shaft about every `8` levels on the wall opposite the ladder with `Copper Torches` when available or ordinary torches otherwise, treats those shaft lights as part of a valid finished shaft level, explores bounded cave breaches off the active shaft instead of freezing when it opens into natural caverns, lights those cave pockets with torches, mines easily exposed nearby ore there, cuts first-pass primary side tunnels from the ladder walls about every `5` levels so adjacent shaft ore can be worked from a standable spot, places `dirt` or `cobblestone` floor supports when a tunnel mouth lacks footing, falls back to mining exposed useful stone at the current shaft face when it cannot safely deepen yet, replaces ladder-support blocks with `cobblestone` or `dirt` when ore extraction would undermine them, and returns up the shaft toward the surface at the end of the workday; later quarries, broader secondary tunnel expansion, and richer refining behavior still remain follow-up work, and upgraded primary tunnels should prefer `Copper Lantern` lighting before falling back to torches
