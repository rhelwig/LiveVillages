...
| `f` | Same as `F`, but use top-half stairs |
...
- The bundled biome palette table currently lives in `data/live-villages/structure_palette_rules.json`.
...
2. Pick one side as the front.
3. Write down the footprint as text, one layer at a time, from ground to roof.
4. Keep the front of the building at the bottom of each text diagram.
5. Add an orientation diagram only where stairs or wall torches need a precise direction.
6. Add a short note for intended materials and bed locations.
