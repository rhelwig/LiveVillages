...
- A planned future extension may add material-preference grids next to placement and orientation grids. These would let a designer request compatible texture variation, such as using mostly stone-family wall blocks while sprinkling in granite, diorite, andesite, or mossy cobblestone, or mixing wood species/forms where the construction intent stays the same.
- Material preferences should be decorative guidance rather than hard requirements. If the settlement lacks the preferred variant, builders should be able to fall back to the normal biome palette or another compatible material so construction does not stall on cosmetic texture choices.
